...
- Producer Console Songs tab now splits songs into an active list and a separate `Archive` section that surfaces `published` / `archived` / `failed` entries with a hint that autopilot does not touch them, so already-released catalog tracks (例: `where-it-played`) sit visually apart from in-flight songs.
